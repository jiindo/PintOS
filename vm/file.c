...
do_mmap (void *addr, size_t length, int writable, struct file *file, off_t offset) {
    ...
    while (read_bytes > 0 || zero_bytes > 0)
    {
        ...
        struct lazy_load_arg *lazy_load_args = (struct lazy_load_arg *)malloc(sizeof(struct lazy_load_arg));
        lazy_load_args->file = f;
        lazy_load_args->ofs = offset;
        lazy_load_args->read_bytes = page_read_bytes;
        lazy_load_args->zero_bytes = page_zero_bytes;

        // vm_alloc_page_with_initializer를 호출하여 대기 중인 객체를 생성합니다.
        if (!vm_alloc_page_with_initializer(VM_FILE, addr,
                                            writable, lazy_load_segment, lazy_load_args))
            return NULL;
